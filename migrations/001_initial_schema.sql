-- =============================================================================
-- Elidune — SQLx migration 001 (initial schema)
-- =============================================================================
-- Canonical schema for fresh databases. Apply with: sqlx migrate run
-- (DATABASE_URL). Same content is linked as database/init_database.sql and
-- scripts/init_database.sql for tooling and docs.
--
...
DO $seed$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM account_types) THEN
...
DO $seed$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM fees) THEN
...
DO $seed$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public_types) THEN
...
DO $seed$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM loans_settings) THEN
...
DO $seed$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM z3950servers) THEN
...
-- =============================================================================
-- SAVED STATS QUERIES (flexible builder)
-- =============================================================================
-- Default shared templates are inserted below when at least one user row exists.
-- Names and JSON payloads are English.
...
DO $sq$
BEGIN
IF NOT EXISTS (SELECT 1 FROM saved_queries) THEN
