...
impl Repository {
    ...
    /// Startup maintenance: strip surrounding ASCII double quotes from series/collection names,
    /// then delete series and collections not linked to any biblio (via `biblio_series` /
    /// `biblio_collections`). Junction rows referencing deleted entities are removed by FK
    /// `ON DELETE CASCADE` on the junction tables.
    pub async fn cleanup_series_collections_at_startup(&self) -> AppResult<()> {
        let mut tx = self.pool.begin().await?;

        let series_names_fixed = sqlx::query(
            r#"
            UPDATE series
        ...
        let collection_names_fixed = sqlx::query(
            r#"
            UPDATE collections
            SET name = trim(both '"' from name),
                updated_at = NOW()
        ...
        let orphan_series_deleted = sqlx::query(
            r#"
        ...
        let orphan_collections_deleted = sqlx::query(
            r#"
        ...
        tracing::info!(
            "startup catalog cleanup: series quoted names fixed={}, collection quoted names fixed={}, orphan series deleted={}, orphan collections deleted={}",
            series_names_fixed,
            collection_names_fixed,
            orphan_series_deleted,
            orphan_collections_deleted,
        );
